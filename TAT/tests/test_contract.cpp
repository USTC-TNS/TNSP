...
TEST(test_contract, no_symmetry_example_0) {
    ...
    auto c_expect =
        TAT::Tensor<double, TAT::NoSymmetry>{{"B", "D"}, {2, 2}}.set_([i = 0, v = std::vector<double>{4, 6, 6, 10}]() mutable { return v[i++]; });
    ASSERT_FLOAT_EQ((c - c_expect).norm<-1>(), 0);

    auto d = TAT::Tensor<double, TAT::NoSymmetry>::contract(a, b, {{"A", "D"}});
    auto d_expect =
        TAT::Tensor<double, TAT::NoSymmetry>{{"B", "C"}, {2, 2}}.set_([i = 0, v = std::vector<double>{2, 6, 3, 11}]() mutable { return v[i++]; });
    ASSERT_FLOAT_EQ((d - d_expect).norm<-1>(), 0);

    auto e = TAT::Tensor<double, TAT::NoSymmetry>::contract(a, b, {{"B", "C"}});
    auto e_expect =
        TAT::Tensor<double, TAT::NoSymmetry>{{"A", "D"}, {2, 2}}.set_([i = 0, v = std::vector<double>{2, 3, 6, 11}]() mutable { return v[i++]; });
    ASSERT_FLOAT_EQ((e - e_expect).norm<-1>(), 0);

    auto f = TAT::Tensor<double, TAT::NoSymmetry>::contract(a, b, {{"B", "D"}});
    auto f_expect =
        TAT::Tensor<double, TAT::NoSymmetry>{{"A", "C"}, {2, 2}}.set_([i = 0, v = std::vector<double>{1, 3, 3, 13}]() mutable { return v[i++]; });
...
TEST(test_contract, no_symmetry_example_1) {
    auto a = TAT::Tensor<double, TAT::NoSymmetry>::contract(
        TAT::Tensor<double, TAT::NoSymmetry>{{"A", "B", "C", "D"}, {1, 2, 3, 4}}.range_(),
        TAT::Tensor<double, TAT::NoSymmetry>{{"E", "F", "G", "H"}, {3, 1, 2, 4}}.range_(),
        {{"B", "G"}, {"D", "H"}}
    );
    auto b = TAT::Tensor<double, TAT::NoSymmetry>{{"A", "C", "E", "F"}, {1, 3, 3, 1}}.set_(
...
TEST(test_contract, fuse) {
    TAT::Size fuse_d = 3;
    TAT::Size common_d = 4;
    auto a = TAT::Tensor<>({"A", "B", "C"}, {fuse_d, common_d, 5}).range_();
    auto b = TAT::Tensor<>({"A", "B", "D"}, {fuse_d, common_d, 7}).range_();
    auto c = TAT::Tensor<>::contract(a, b, {{"B", "B"}}, {"A"});
    for (TAT::Size i = 0; i < fuse_d; i++) {
        auto hat = TAT::Tensor<>({"A"}, {fuse_d}).zero_();
        hat.storage()[i] = 1;
...
TEST(test_contract, corner_no_symmetry_0k) {
    auto a = TAT::Tensor<double, TAT::NoSymmetry>{{"A", "B"}, {2, 0}}.range_();
    auto b = TAT::Tensor<double, TAT::NoSymmetry>{{"C", "D"}, {0, 2}}.range_();

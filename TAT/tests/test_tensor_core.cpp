...
TEST(test_tensor_core, create_core_u1_symmetry) {
    const auto b = TAT::Core<float, TAT::BoseU1Symmetry>({{{0, 2}, {1, 3}}, {{0, 4}, {-1, 5}}});
    auto e0_b = TAT::Edge<TAT::BoseU1Symmetry>({{0, 2}, {1, 3}});
    auto e1_b = TAT::Edge<TAT::BoseU1Symmetry>({{0, 4}, {-1, 5}});
    ...
    ASSERT_EQ(&b.blocks(std::vector<TAT::BoseU1Symmetry>{0, 0}), &b.blocks(std::vector<int>{0, 0}));
    ASSERT_EQ(&b.blocks(std::vector<TAT::BoseU1Symmetry>{1, -1}), &b.blocks(std::vector<int>{1, 1}));
    ASSERT_EQ(&b.at(std::vector<TAT::Size>{1, 3}), &b.at(std::vector<std::pair<TAT::Nums, TAT::Size>>{{0, 1}, {0, 3}}));
    ASSERT_EQ(&b.at(std::vector<TAT::Size>{3, 6}), &b.at(std::vector<std::pair<TAT::BoseU1Symmetry, TAT::Size>>{{1, 1}, {-1, 2}}));
}

TEST(test_tensor_core, create_core_u1_symmetry_empty_blocks) {
    auto c = TAT::Core<float, TAT::BoseU1Symmetry>({{}, {}});
    auto e0_c = TAT::Edge<TAT::BoseU1Symmetry>();
    auto e1_c = TAT::Edge<TAT::BoseU1Symmetry>();

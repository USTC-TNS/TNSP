...
def configuration(state):
    L1 = state.L1
    L2 = state.L2
    F = state.total_symmetry.fermi
    U = F // 2
    D = F - U
    for l1 in range(L1):
        for l2 in range(L2):
            for o in range(2):
                if (l1, l2, o) == (0, 0, 0) or (l1, l2, o) == (L1 - 1, L2 - 1, 1):
                    continue
                state.configuration[l1, l2, o] = (0, 0)
    randL1 = TAT.random.uniform_int(0, L1 - 1)
    randL2 = TAT.random.uniform_int(0, L2 - 1)
    rando = TAT.random.uniform_int(0, 1)
    u = 0
    while u < U:
        l1 = randL1()
        l2 = randL2()
        ...
        if (l1, l2, o) == (0, 0, 0) or (l1, l2, o) == (L1 - 1, L2 - 1, 1):
            continue
        if state.configuration[l1, l2, o] == (0, 0):
            state.configuration[l1, l2, o] = (1, 0)
            u += 1
        elif state.configuration[l1, l2, o] == (1, 1):
            state.configuration[l1, l2, o] = (2, 0)
            u += 1
    d = 0
    while d < D:
        l1 = randL1()
        ...
        if (l1, l2, o) == (0, 0, 0) or (l1, l2, o) == (L1 - 1, L2 - 1, 1):
            continue
        if state.configuration[l1, l2, o] == (0, 0):
            state.configuration[l1, l2, o] = (1, 1)
            d += 1
        elif state.configuration[l1, l2, o] == (1, 0):
            state.configuration[l1, l2, o] = (2, 0)
            d += 1

...
namespace TAT {
   ...
   void report_ptr_edge(const vector<PtrEdge<Symmetry>>& edges) {
      for (const auto& e : edges) {
         for (const auto& [sym, dim] : *e.map) {
            std::cout << sym << ':' << dim << ',' << ' ';
         }
         if constexpr (is_fermi_symmetry_v<Symmetry>) {
            std::cout << ' ' << e.arrow << e.reverse_mark << '\n';
         }
      }
   }
#endif
   ...
   [[nodiscard]] Tensor<ScalarType, Symmetry> Tensor<ScalarType, Symmetry>::edge_operator(
            ...
            for (auto j = start_of_merge; j < end_of_merge; j++) {
               if constexpr (is_fermi_symmetry_v<Symmetry>) {
                  if (arrow_fixed) {
                     ...
                     if (edge_5[j].arrow_valid()) {
                        arrow_fixed = true;
                        arrow = edge_5[j].arrow ^ edge_5[j].reverse_mark;
                        reversed_flag_dst.push_back(false);
                     } else {
                        reversed_flag_dst.push_back(false);
                     }
      ...
      if (!merge_map.empty()) {
         ...
         for (auto& [sym, size] : symmetries_dst) {
            ...
            true_data_5[std::move(sym)] = offset_dst[target_symmetry];
...
#if 0
         std::clog << ptr_src << ',' << ptr_dst << ',' << total_size << '\n';
#endif

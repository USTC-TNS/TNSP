...
#pragma once
#ifndef TAT_MULTIDIMENSION_SPAN_HPP
#define TAT_MULTIDIMENSION_SPAN_HPP
...
namespace TAT {
    ...
    void mdspan_transform(const mdspan<T1, U1>& source, mdspan<T2, U2>& destination, Func&& func) {
        if constexpr (debug_mode) {
            if (!std::equal(
                    source.dimensions().begin(),
                    source.dimensions().end(),
                    destination.dimensions().begin(),
                    destination.dimensions().end()
                )) {
                detail::error("transform data between mdspan where dimension are not the same");
            }
        }
    ...
    void matrix_transpose(Size m, Size n, const T* source, T* destination) {
        // source m*n
        // destination n*m
        detail::mdspan_transform_kernel(source, destination, Vector{n, m}, Vector{1, n}, Vector{m, 1}, [](const auto& x) { return x; });
    }
} // namespace TAT

...
namespace TAT {
   ...
   struct scope_resource {
      ...
      detail::pmr::memory_resource* upstream;
...
namespace TAT {
   namespace no_initialize {
      ...
      namespace pmr {
          ...
          */
         template<typename T>
         using vector = std::vector<T, detail::pmr::no_initialize_polymorphic_allocator<T>>;
      } // namespace pmr
   ...
   namespace pmr {
      ...
      template<typename T>
      using vector = std::vector<T, detail::pmr::polymorphic_allocator<T>>;

      template<typename T>
      using list = std::list<T, detail::pmr::polymorphic_allocator<T>>;

      template<typename Key, typename T, typename Compare = std::less<Key>>
      using map = std::map<Key, T, Compare, detail::pmr::polymorphic_allocator<std::pair<const Key, T>>>;

      template<class Key, class Compare = std::less<Key>>
      using set = std::set<Key, Compare, detail::pmr::polymorphic_allocator<Key>>;
   } // namespace pmr
} // namespace TAT
#endif

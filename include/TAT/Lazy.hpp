...
namespace TAT {
  /*
  namespace lazy {
    template<device=Device::CPU, class Base=double>
    class Lazy;
  } // namespace lazy
  using lazy::Lazy;
  */
  ...
  namespace lazy {
    ...
    class Lazy final : public BaseLazy {
     public:
      std::unique_ptr<Node<Base>> node;
      std::function<Node<Base>()> func;
      ...
      std::shared_ptr<Lazy<Base>> shared_from_this() {
        return std::dynamic_pointer_cast<Lazy<Base>>(std::enable_shared_from_this<BaseLazy>::shared_from_this());
      } // shared_from_this

      template<class ... Args>
      std::shared_ptr<Lazy<Base>> set(Args&& ... args) {
        reset(); // since new node set, lazy need reset
        node = std::unique_ptr<Node<Base>>(new Node<Base>(std::forward<Args>(args) ...));
        return shared_from_this();
      } // set_lazy, update node

      template<class T1=std::vector<Legs>, class T2=std::vector<Size>>
      std::shared_ptr<Lazy<Base>> set(T1&& _legs, T2&& _dims) {
        reset(); // since new node set, lazy need reset
        node = std::unique_ptr<Node<Base>>(new Node<Base>(std::forward<T1>(_legs), std::forward<T2>(_dims)));
        return shared_from_this();
      } // set_lazy, update node

      template<class ... Args>
      static std::shared_ptr<Lazy<Base>> make(Args&& ... args) {
        auto res = std::make_shared<Lazy<Base>>();
        res->set(std::forward<Args>(args) ...);
        return res;
      } // make_lazy by initial node

      template<class T1=std::vector<Legs>, class T2=std::vector<Size>>
      static std::shared_ptr<Lazy<Base>> make(T1&& _legs, T2&& _dims) {
        auto res = std::make_shared<Lazy<Base>>();
        res->set(std::forward<T1>(_legs), std::forward<T2>(_dims));
        return res;
      } // make_lazy by initial node

      std::shared_ptr<Lazy<Base>> set_test() {
        reset(false);
        node->set_test();
        return shared_from_this();
      } // set_test
      std::shared_ptr<Lazy<Base>> set_zero() {
        reset(false);
        node->set_zero();
        return shared_from_this();
      } // set_zero
      std::shared_ptr<Lazy<Base>> set_random(const std::function<Base()>& random) {
        reset(false);
        node->set_random(random);
        return shared_from_this();
      } // set_random
      std::shared_ptr<Lazy<Base>> set_constant(Base num) {
        reset(false);
        node->set_constant(num);
        return shared_from_this();
      } // set_constant

      std::shared_ptr<Lazy<Base>> calc() {
        if (!node) {
          set(std::move(func()));
        } // has no value
        return shared_from_this();
      ...
      std::shared_ptr<Lazy<Base>> legs_rename(const std::map<Legs, Legs>& dict) {
        reset(false);
        if (node) {
      ...
      template<int n>
      std::shared_ptr<Lazy<Base>> normalize() {
        reset(false);
        if (node) {
      ...
      std::shared_ptr<Lazy<Base>> transpose(const std::vector<Legs>& new_legs) {
        auto res = std::make_shared<Lazy<Base>>();
        auto origin = shared_from_this();
        res->func = [=]() {
      ...
      static std::shared_ptr<Lazy<Base>> contract(std::shared_ptr<Lazy<Base>> lazy1,
                                              std::shared_ptr<Lazy<Base>> lazy2,
      ...
      const std::map<Legs, Legs>& map2) {
        auto res = std::make_shared<Lazy<Base>>();
        res->func = [=]() {
          return Node<Base>::contract(lazy1->value(), lazy2->value(), legs1, legs2, map1, map2);
        };
      ...
      std::shared_ptr<Lazy<Base>> multiple(std::shared_ptr<Lazy<Base>> other, const Legs& position) {
        auto res = std::make_shared<Lazy<Base>>();
        auto origin = shared_from_this();
        res->func = [=]() {

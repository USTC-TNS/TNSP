        ...
        const scalar_t& at(const std::vector<T, A>& arg) const {
            ...
            for (auto i = 0; i < blocks().rank(); i++) {
                auto [position, offset] = overloaded{
                    [](const edge_t& edge, const typename edge_t::coord_t& coord) { return coord; },
                    [](const edge_t& edge, const typename edge_t::index_t& index) { return edge.coord_by_index(index); },
                    [](const edge_t& edge, const typename edge_t::point_t& point) { return edge.coord_by_point(point); }}(edges(i), arg[i]);

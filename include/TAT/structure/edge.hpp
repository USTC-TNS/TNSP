   ...
   class edge_segments_t {
    ...
    public:
      auto find_by_symmetry(const symmetry_t& symmetry) const {
         return std::find_if(segments().begin(), segments().end(), [&symmetry](const auto& x) {
    ...
    public:
      ...
      template<
            ...
            typename = std::enable_if_t<
                  std::is_constructible_v<base_segments_t, Arg&&> && !std::is_same_v<remove_cvref_t<Arg>, Edge<Symmetry, is_pointer>>>>

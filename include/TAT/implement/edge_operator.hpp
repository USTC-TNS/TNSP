...
namespace TAT {
    ...
    Tensor<ScalarType, Symmetry, Name> Tensor<ScalarType, Symmetry, Name>::edge_operator_implement(
        ...
        const H& edges_and_symmetries_to_cut_before_all
    ) const {
        ...
        if (merge_map.size() != 0) {
            ...
            for (Rank index_after_merge = 0, start_of_merge = 0, end_of_merge = 0; index_after_merge < rank_after_merge; index_after_merge++) {
                ...
                if (end_of_merge != 1 + start_of_merge) {
                    ...
                    for (auto& [symmetry, position, size, offset, parity] : this_offset_pool) {
                        auto found =
                            std::find_if(merged_edge.begin(), merged_edge.end(), [&sym = symmetry](const auto& pair) { return pair.first == sym; });
        ...
        for (auto it = main_loop.begin(); it.valid; ++it) {
            ...
            const auto& positions_before_cut =
                edges_and_symmetries_to_cut_before_all.size() != 0 ? real_positions_before_cut : positions_before_split;
            ...
            for (auto index_after_split = rank_at_transpose; index_after_split-- > 0;) {
                if (index_after_split != rank_at_transpose - 1 && split_flags[index_after_split] == split_flags[index_after_split + 1]) {
                    leadings_after_split[index_after_split] =
                        leadings_after_split[index_after_split + 1] * dimensions_after_split[index_after_split + 1];
                } else {
            ...
            if (total_parity) {
                mdspan_transform(source_span.transpose(plan_destination_to_source), destination_span, [](const auto& x) { return -x; });
            } else {
                mdspan_transform(source_span.transpose(plan_destination_to_source), destination_span, [](const auto& x) { return x; });
            }
        }

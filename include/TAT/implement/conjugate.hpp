...
namespace TAT {
    ...
    Tensor<ScalarType, Symmetry, Name> Tensor<ScalarType, Symmetry, Name>::conjugate(bool trivial_metric) const {
        ...
        if constexpr (Symmetry::length == 0) {
            if constexpr (is_real<ScalarType>) {
                ...
                return map([](const auto& x) { return std::conj(x); });
            }
        }
        ...
        if constexpr (Symmetry::is_fermi_symmetry) {
            for (auto i = 0; i < rank(); i++) {
                const bool trivial_metric_with_arrow_true = edges(i).arrow() && trivial_metric;
                ...
                for (Size x = 0; x < out_size; x++) {
                    ...
                    for (Size y = 0; y < self_size; y++) {
                        ...
                        for (Size z = 0; z < in_size; z++) {
                            ...
                            signs_pool[offset_for_z].second ^= trivial_metric_with_arrow_true;
                        }
                    }
                }
            }

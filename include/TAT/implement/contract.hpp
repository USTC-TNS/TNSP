...
namespace TAT {
   ...
   Tensor<ScalarType, Symmetry, Name> contract_without_fuse(
      ...
      for (auto iterator = contract_names.begin(); iterator != contract_names.end();) {
         auto found_1 = tensor_1.find_rank_from_name(std::get<0>(*iterator));
         auto found_2 = tensor_2.find_rank_from_name(std::get<1>(*iterator));
         if (found_1 == tensor_1.names.end() || found_2 == tensor_2.names.end()) {
   ...
   Tensor<ScalarType, Symmetry<>, Name> contract_with_fuse(
      ...
      for (const auto& name : tensor_1.names) {
         const auto in_tensor_2 = tensor_2.find_rank_from_name(name) != tensor_2.names.end();

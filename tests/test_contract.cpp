...
TEST(test_contract, no_symmetry_example_0) {
   ...
   auto c_expect = TAT::Tensor<double, TAT::NoSymmetry>{{"B", "D"}, {2, 2}}.set([i = 0, v = std::vector<double>{4, 6, 6, 10}]() mutable {
      return v[i++];
   });
   ASSERT_FLOAT_EQ((c - c_expect).norm<-1>(), 0);

   auto d = TAT::Tensor<double, TAT::NoSymmetry>::contract(a, b, {{"A", "D"}});
   auto d_expect = TAT::Tensor<double, TAT::NoSymmetry>{{"B", "C"}, {2, 2}}.set([i = 0, v = std::vector<double>{2, 6, 3, 11}]() mutable {
      return v[i++];
   });
   ASSERT_FLOAT_EQ((d - d_expect).norm<-1>(), 0);

   auto e = TAT::Tensor<double, TAT::NoSymmetry>::contract(a, b, {{"B", "C"}});
   auto e_expect = TAT::Tensor<double, TAT::NoSymmetry>{{"A", "D"}, {2, 2}}.set([i = 0, v = std::vector<double>{2, 3, 6, 11}]() mutable {
      return v[i++];
   });
   ASSERT_FLOAT_EQ((e - e_expect).norm<-1>(), 0);

   auto f = TAT::Tensor<double, TAT::NoSymmetry>::contract(a, b, {{"B", "D"}});
   auto f_expect = TAT::Tensor<double, TAT::NoSymmetry>{{"A", "C"}, {2, 2}}.set([i = 0, v = std::vector<double>{1, 3, 3, 13}]() mutable {
      return v[i++];
   });
...
TEST(test_contract, no_symmetry_example_1) {
   ...
   auto b = TAT::Tensor<double, TAT::NoSymmetry>{{"A", "C", "E", "F"}, {1, 3, 3, 1}}.set(
         [i = 0, v = std::vector<double>{316, 796, 1276, 428, 1164, 1900, 540, 1532, 2524}]() mutable {
            return v[i++];
         });
}

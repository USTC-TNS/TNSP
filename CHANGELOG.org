...
+ *tetragono*: Fix the first order bias of the observables and the gradient given by the sampling method.
+ *tetragono*: Fix the termination condition of conjugate gradient method when the matrix is rank deficient.

...
+ *tetraux*: Add a function =pseudo_inverse_kernel= used to speed up min-SR method via scalapack.
*** Changed
+ *tetragono*: Use the new function =pseudo_inverse_kernel= in =tetraux= to speed up min-SR method which using
  scalapack. Only when compiling tetraux with scalapack, this function will be compiled. Without scalapack support, the
  min-SR method can not run parallelly.
...
+ *TAT.py*: Add =dtype= class member for =Tensor= object, which makes it easier to communicate with numpy.
*** Deprecated
*** Removed
*** Fixed

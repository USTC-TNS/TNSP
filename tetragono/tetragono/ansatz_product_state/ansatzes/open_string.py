...
class OpenString(AbstractAnsatz):
    ...
    def weight_and_delta(self, configurations, calculate_delta):
        ...
        for index in range(self.length):
            if index == 0:
                holes[index] = right[index + 1].contract(hat_list[index], set(), {"C"}).edge_rename({"L": "R"})
            elif index == self.length - 1:
                holes[index] = left[index - 1].contract(hat_list[index], set(), {"C"}).edge_rename({"R": "L"})
            else:
                holes[index] = (
                    left[index - 1]  #
                    .contract(right[index + 1], set(), {"C"})  #

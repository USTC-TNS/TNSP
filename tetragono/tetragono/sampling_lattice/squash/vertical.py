...
def unsquash(new_state, old_state, cut_dimension):
    ...
    for l1, l2 in new_state.sites():
        ...
        left_split = []
        if "L" in part1.names:
            left_split.append(("L1", part1.edges("L").segment))
        if "L" in part2.names:
            left_split.append(("L2", part2.edges("L").segment))
        down_split = []
        if "R" in part1.names:
            down_split.append(("R1", part1.edges("R").segment))
        if "R" in part2.names:
            down_split.append(("R2", part2.edges("R").segment))
        split_plan = {}
        if left_split:
        ...
        delta = np.sqrt(np.abs(singular.storage))
        delta[delta == 0] = 1
        singular.storage /= delta

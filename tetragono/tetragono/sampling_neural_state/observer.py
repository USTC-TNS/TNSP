...
class Observer():
    ...
    def __call__(self, configurations, amplitudes, weights):
        """
        ...
        for batch_index in range(batch_size):
            ...
            amplitude = amplitudes[batch_index]
            weight = weights[batch_index]

            ws = Configuration(self.owner).import_configuration(configuration).basis() * amplitude
            if ws.norm_max() == 0:
                continue
            ...
            for name, observers in self._observer.items():
                for positions, observer in observers.items():
                    ...
                    for positions_configuration_s, observer_shrinked in element_pool[positions_configuration].items():
                        configuration_s = configuration.clone()
                        ...
                        wss = Configuration(self.owner).import_configuration(configuration_s).basis()
                        if wss.norm_max() == 0:
                            continue
                        value = (
                            inv_ws_conj  #
                        ...
                        if torch.equal(configuration_s, configuration):
                            result[batch_index][name][positions] += amplitude.item() * complex(value)
                            whole_result[batch_index][name] += amplitude.item() * complex(value)
                        else:
                            params_and_targets.append((complex(value), (batch_index, name, positions)))
                            configurations_s.append(configuration_s)

        if len(configurations_s) != 0:
            amplitudes_s = self.owner(torch.stack(configurations_s, dim=0), enable_grad=False)
            for amplitude_s, [param, [batch_index, name, positions]] in zip(amplitudes_s, params_and_targets):
                result[batch_index][name][positions] += amplitude_s.item() * param
                whole_result[batch_index][name] += amplitude_s.item() * param
        ...
        for batch_index in range(batch_size):
            ...
            self._total_log_ws += amplitude.abs().log().item()
            ...
            for name, observers in self._observer.items():
                ...
                if name == "energy":
                    self._total_imaginary_energy_reweight += whole_result[batch_index][name].imag * reweight
                if name == "energy" and self._enable_gradient:
                    ...
                    if self._Delta is None:
                        self._Delta = holes * reweight
                    else:
                        self._Delta += holes * reweight
                    if self._EDelta is None:
                        self._EDelta = holes * reweight * Es
                    else:
                        self._EDelta += holes * reweight * Es
                    if self._enable_natural:
    ...
    def natural_gradient(self, step=None, error=None, epsilon=None):
        ...
        Energy = torch.tensor(Energy, device=Delta.device, dtype=Delta.dtype).conj()

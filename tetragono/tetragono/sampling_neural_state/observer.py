...
class Observer():
    ...
    def _fermi_sign(self, config, config_s, positions):
        ...
        count = 0
        for pos in (((l1 * L2) + l2) * orbit + o for l1, l2, o in positions):
            # Since op(_s)[pos] may be updated during the loop, we cannot sum them before loop.
            count += torch.sum(op[:pos]) * op[pos]
            op[pos] = False
            count += torch.sum(op_s[:pos]) * op_s[pos]
            op_s[pos] = False
        if count % 2 == 0:
            return +1
        else:

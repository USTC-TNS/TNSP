...
def check_difference(state, observer, grad, energy_observer, configuration_pool, check_difference_delta):
    ...
    def get_energy():
        with energy_observer:
            for configurations, _, weights in configuration_pool:
                amplitudes = state(configurations)
                energy_observer(configurations, amplitudes, weights)
    ...
    for i in range(len(grad)):
        ...
        if state.Tensor.is_complex:
            state.apply_gradient(basis, -1j * delta + delta)
            now_energy = get_energy()
            ...
            cgrad = rgrad
        state.apply_gradient(basis, +delta)
        showln(grad[i] / cgrad, cgrad, grad[i])
    ...
    for grad_step in range(grad_total_step):
        ...
        with seed_differ, observer:
            ...
            for sampling_step in range(0, sampling_total_step_rank, sampling_batch_size):
                ...
                observer(configurations, amplitudes, weights)
                if need_energy_observer:
                    configuration_pool.append((configurations.clone(), amplitudes, weights))
                show(f"sampling {100*sampling_step/sampling_total_step:.2f}%, energy={observer.energy}")
        showln(f"sampling done, total_step={observer.count}, energy={observer.energy}")
        ...
        if save_configuration_file:
            write_configurations(sampling_configurations.cpu(),

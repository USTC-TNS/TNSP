...
def gradient_descent(
    ...
    sampling_batch_size : int, default = +1
        The sampling batch size for single sampling.
    observe_batch_size : int, default = +1
    ...
    base_sampling_batch_size = sampling_batch_size
    base_observing_batch_size = observing_batch_size
    ...
    for grad_step in range(grad_total_step):
        ...
        with seed_differ, observer:
            ...
            if sampling_method == "sweep":
                ...
                if sampling_batch_size is not 1:  # Default as 1
                    assert sampling_batch_size == len(sampling_configurations)
                sampling = SweepSampling(state, torch.tensor(sampling_configurations, device=state.device), sweep_alpha,
                ...
                sampling_total_step_rank = sampling_total_step // mpi_size + (mpi_rank < sampling_total_step % mpi_size)
                if base_sampling_batch_size == -1:
                    sampling_batch_size = sampling_total_step_rank
                sampling = DirectSampling(state, sweep_alpha, sampling_batch_size)
                sampling_total_step = sampling_total_step
            elif sampling_method == "ergodic":
                sampling = ErgodicSampling(state, sampling_batch_size)
            ...
            for sampling_step in range(0, sampling_total_step_rank, sampling_batch_size):
                process = sampling_step / sampling_total_step_rank
                show(f"sampling {100*process:.2f}%")
                batch_size = min(sampling_batch_size, sampling_total_step_rank - sampling_step)
                configurations, amplitudes, weights, multiplicities = sampling()
                assert multiplicities.sum().cpu() == sampling_batch_size
                ...
                multiplicities = multiplicities[:batch_size]
                configurations_pool.append(configurations)
                amplitudes_pool.append(amplitudes)
                weights_pool.append(weights)
                multiplicities_pool.append(multiplicities)
            configurations_pool = torch.cat(configurations_pool)
            amplitudes_pool = torch.cat(amplitudes_pool)
            weights_pool = torch.cat(weights_pool)
            multiplicities_pool = torch.cat(multiplicities_pool)
            ...
            if base_observing_batch_size == -1:
                observing_batch_size = unique_sampling_count
            for unique_sampling_step in range(0, unique_sampling_count, observing_batch_size):
                process = unique_sampling_step / unique_sampling_count
                show(f"observing {100*process:.2f}%, energy={observer.energy}")
                configurations = configurations_pool[unique_sampling_step:unique_sampling_step + observing_batch_size]
                amplitudes = amplitudes_pool[unique_sampling_step:unique_sampling_step + observing_batch_size]
                weights = weights_pool[unique_sampling_step:unique_sampling_step + observing_batch_size]
                multiplicities = multiplicities_pool[unique_sampling_step:unique_sampling_step + observing_batch_size]

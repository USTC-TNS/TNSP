...
utility.py contains several utility function.
auxiliary.py which make use of lazy.py, used in configuration type, contains auxiliary system which reduce duplicated calculation when contract tensor network with several site removed or replaced.
tensor_element.py is used to make hamiltonian sparse, which is used in sampling method.
conversion.py defines conversion function among exact_state, simple_update_lattice and sampling_lattice.
After all, shell.py provide a high-level, user-friendly interface for all of the function in tetragono.py
...
Finally, shell.py offers a high-level, user-friendly interface for accessing all functions within tetragono.py.

...
def abstract_state(L1, L2, mu, side=1):
    """
    Create density matrix of free fermion model state.
    ...
    for l1 in range(L1):
        for l2 in range(L2):
            state.physics_edges[(l1, l2, 0)] = [(False, 1), (True, 1)]
            state.physics_edges[(l1, l2, 1)] = [(False, 1), (True, 1)]
...
def abstract_lattice(L1, L2, mu, side=1, D=1):
    """
    Create density matrix of free fermion model lattice.
    ...
    """
    state = tet.AbstractLattice(abstract_state(L1, L2, mu, side=side))
    if D == 1:
        edge = [(False, 1)]
    else:
        D1 = D // 2

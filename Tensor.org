...
Let $r_s = \frac{\langle \psi|s|\psi\rangle}{p_s}$, $E_s = \frac{\sum_{s'}\langle\psi|s|H|s'|\psi\rangle}{\langle\psi|s|\psi\rangle}$,
We have $E = \frac{\sum_s p_s r_s E_s}{\sum_s p_s r_s}$,
The sampling method gives the energy as
$\hat E = \frac{\sum_s q_s r_s E_s}{\sum_s q_s r_s}$.

let $\langle O \rangle_p=\sum_{s}O(s)p(s)$, so $E=\frac{\langle r E\rangle_p}{\langle r \rangle_p}$,
Ignore $O(1/n^2)$, we have
$\langle \hat E - E \rangle = \frac{1}{n}\frac{\left\langle r^2\right\rangle  E - \left\langle E r^2\right\rangle }{\langle r\rangle ^2}$
, so this is a biased estimator in fact.

As for the variance,
$\langle (\hat E - E)^2 \rangle =\frac{1}{n}\left( \frac{\langle r^2 \rangle_p \langle rE \rangle_p^2}{\langle r \rangle_p^4}-\frac{2 \langle r^2E \rangle_p \langle rE \rangle_p}{\langle r \rangle_p^3}+\frac{\langle r^2E^2 \rangle_p}{\langle r \rangle_p^2} \right)$.
let $\langle O(s) \rangle_\psi = \frac{\langle O(s) r_s \rangle_p}{\langle r_s \rangle_p}$, $r'_s=\frac{r_s}{\langle r\rangle_p}$, the former formula could be written as
$\langle (\hat E - E)^2 \rangle =\frac{1}{n}\left\langle (E - \langle E \rangle_\psi)^2 r' \right\rangle_\psi$.

The code to get the result of variance in mathematica is followed
#+begin_src mathematica :exports code
exactE = Sum[p[ss] r[ss] e[ss], {ss, S}]/Sum[p[ss] r[ss], {ss, S}];
hatE = Sum[q[ss] r[ss] e[ss], {ss, S}]/Sum[q[ss] r[ss], {ss, S}];
dd = D[(hatE - exactE)^2, q[s], q[t]];
Expect = Simplify[
  1/(2 n) Sum[
     Expand[(KroneckerDelta[s, t] p[s] - p[s] p[t]) dd], {s, S}, {t,
      S}] //. {
    (*q is set to p after derivate*)
    q[s_] :> p[s],
    (*Simplify delta*)
    Sum[a_ KroneckerDelta[i_, j_], rb___, {j_, S}, re___] :>
     If[Length[{re, rb}] != 0, Sum[a /. j -> i, rb, re],
      a /. j -> i],
    (*Improve readability of result*)
    Sum[p[ss_] r[ss_] e[ss_], {ss_, S}] ->
     AngleBracket[r \[CapitalEpsilon]],
    Sum[p[ss_] r[ss_], {ss_, S}] -> AngleBracket[r],
    Sum[p[ss_] r[ss_]^2 e[ss_], {ss_, S}] ->
     AngleBracket[r^2 \[CapitalEpsilon]],
    Sum[p[ss_] r[ss_]^2, {ss_, S}] -> AngleBracket[r^2],
    Sum[p[ss_] r[ss_]^2 e[ss_]^2, {ss_, S}] ->
     AngleBracket[r^2 \[CapitalEpsilon]^2],
    (*Move symbol or number out from sum*)
    Sum[a_ b_?NumericQ, r__] :> b Sum[a, r],
    Sum[a_ b_?NumericQ, r__] :> b^n Sum[a, r],
    Sum[a_ b_AngleBracket, r__] :> b Sum[a, r],
    Sum[a_ b_AngleBracket^n_, r__] :> b^n Sum[a, r],
    Sum[a_/b_AngleBracket, r__] :> 1/b Sum[a, r],
    Sum[a_/b_AngleBracket^n_, r__] :> 1/b^n Sum[a, r],
    (*Expand sum*)
    Sum[a_Plus, b__] :> (Sum[#, b] & /@ a),
    Sum[u_Times, {s_, S}, {t_, S}] :>
     Sum[Select[u, #[[1]] == s &], {s, S}] Sum[
       Select[u, #[[1]] == t &], {t, S}]
    }]
#+end_src
...
where $\langle O(s) \rangle_\psi = \frac{\langle O(s) r_s \rangle_p}{\langle r_s \rangle_p}$, $G_s = (\Delta_s-\langle \Delta\rangle_\psi)(E_s-\langle E\rangle_\psi)$, $r'_s=\frac{r_s}{\langle r\rangle_p}$
and gradient can also be written as $G=\langle (\Delta-\langle \Delta \rangle_\psi) (E - \langle E \rangle_\psi)\rangle_\psi=\langle G \rangle_\psi$
